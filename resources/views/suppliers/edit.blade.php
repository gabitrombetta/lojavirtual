    ...
    <div class="max-w-xl mx-auto bg-white dark:bg-gray-800 p-6 rounded-md shadow">
        ...
        <form action="{{ url('suppliers/update') }}" method="POST">
                ...
                <div class="flex items-center gap-4">
                    <label class="flex items-center text-sm text-gray-700 dark:text-gray-300">
                        <input
                            ...
                            {{ (old('type') ?? $supplier['type'] ?? '') === 'F' ? 'checked' : '' }}>
                    ...
                    <label class="flex items-center text-sm text-gray-700 dark:text-gray-300">
                        <input
                            ...
                            value="J"
                            class="border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500"
                            {{ (old('type') ?? $supplier['type'] ?? '') === 'J' ? 'checked' : '' }}>
            ...
            <div>
                ...
                <x-text-input class="w-full" type="text" name="name" value="{{ $supplier['name'] }}"/>
            </div>

            <div>
                <x-input-label for="name" :value="__('CPF/CNPJ')" />
                <x-text-input class="w-full" type="text" name="cpf_cnpj" value="{{ $supplier['cpf_cnpj'] }}"/>
            </div>

            <div>
                <x-input-label for="name" :value="__('Telefone')" />
                <x-text-input class="w-full" type="text" name="phone" value="{{ $supplier['phone'] }}"/>
            </div>
